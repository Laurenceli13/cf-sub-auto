"""Node speed test: TCP latency scoring for all fetched proxy nodes.
...
nodes_status.json for the dashboard, and keeps only the top 100 most
...
TIMEOUT_FIRST = 5.0  # TCP connect timeout for pass 1 (generous)
TIMEOUT_SECOND = 5.0  # TCP connect timeout for pass 2
...
def main() -> None:
    ...
    # Parse hosts — deduplicate by host:port
    seen = set()
    ...
    # ── Pass 1: generous timeout ──
    results_pass1 = run_single_pass(targets, TIMEOUT_FIRST, MAX_WORKERS)
    pass1_alive = sum(1 for _, p, _ in targets if results_pass1.get(f"{_}:{p}") is not None for _, _, _ in [(h, p, l) for h, p, l in targets])
    pass1_alive = sum(1 for h, p, _ in targets if results_pass1.get(f"{h}:{p}") is not None)
    print(f"pass 1 done: {pass1_alive}/{len(targets)} alive (timeout={TIMEOUT_FIRST}s)")

    # ── Pass 2: re-test survivors ──
    pass2_targets = [
    ...
    # Combine: TCP pass2 must succeed
    nodes = []
